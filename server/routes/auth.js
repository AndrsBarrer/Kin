...
router.post('/magic-link', async (req, res, next) => {
  try {
    const { email, displayName, claimToken, treeId, createAccount, locale } = req.body;
    if (!email) return res.status(400).json({ error: 'Email is required' });

    const { magicLink } = await createMagicLink(email, { displayName, claimToken, treeId, createAccount, locale });

    await sendMagicLinkEmail(email, magicLink, { displayName, claimToken, treeId, locale });
